...
class UpstreamTracker:
    ...
    def __init__(self, aios_root: Path):
        ...
        self.tracking_state_file = (
            aios_root / "ai" / "tools" / ".upstream_tracking_state.json"
        )
    ...
    def git_log_since(
        self, repo_path: Path, since_commit: Optional[str] = None
    ) -> List[UpstreamCommit]:
    ...
    def _analyze_extraction_opportunity(self, commit: UpstreamCommit):
        ...
        if keyword_matches or relevant_files:
            ...
            is_high_priority = any(
                re.search(pattern, message_lower) for pattern in high_priority_patterns
            )
    ...
    def check_repository(self, repo_name: str) -> UpstreamTrackingState:
        ...
        try:
            subprocess.run(
                ["git", "fetch"], cwd=repo_path, check=True, capture_output=True
            )
        except subprocess.CalledProcessError as e:
        ...
        state = UpstreamTrackingState(
            ...
            next_check_recommended=(datetime.now(timezone.utc))
            .replace(day=datetime.now().day + 7)
            .isoformat(),
        )
    ...
    def generate_extraction_report(
        self, repo_name: str, commits: List[UpstreamCommit]
    ) -> str:
        ...
        high_priority = [c for c in extraction_opps if c.extraction_priority == "HIGH"]
        medium_priority = [
            c for c in extraction_opps if c.extraction_priority == "MEDIUM"
        ]
...
def main():
    ...
    if state.extraction_opportunities > 0:
        ...
        report_path = (
            ...
            / "docs"
            / "integration"
            / f"UPSTREAM_REPORT_{repo_name}_{datetime.now().strftime('%Y%m%d')}.md"
        )
