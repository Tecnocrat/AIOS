...
class AIOSGemmaBridge:
    ...
    def __init__(self, model: str = GEMMA_MODEL):
        ...
        self.fast_patterns = {
            SignalType.E501_STRING: re.compile(r'["\'][^"\']{50,}["\']'),
            SignalType.E501_PATH: re.compile(
                r'Path\s*\([^)]{50,}\)|["\'][A-Za-z]:\\[^"\']{40,}["\']'
            ),
            SignalType.E501_IMPORT: re.compile(r"^from\s+\S+\s+import\s+.{50,}$"),
            SignalType.E501_DICT: re.compile(r"\{[^}]{60,}\}"),
            SignalType.E501_CALL: re.compile(r"\w+\([^)]{50,}\)"),
        }
    ...
    async def _call_gemma(self, prompt: str, temperature: float = 0.1) -> str:
        """
...
ANSWER:"""
        ...
        try:
            ...
            self.stats["errors"] += 1
            return GemmaResponse(
                success=False, signal=None, raw_response="", error=str(e)
            )
