...
class SemanticCompressionEngine:
    ...
    def __init__(self):
        ...
        self.delimitation_rules = {
            ...
            r"namespace|dilution|localization))": self._domain_specific_replacement,
        }
    ...
    def apply_compression(
        ...
        for pattern, replacement_func in self.delimitation_rules.items():
            ...
            if matches:
                content = re.sub(
                    pattern, replacement_func, content, flags=re.IGNORECASE
                )
                changes_made.append(
                    {
                        "pattern": pattern,
    ...
    def generate_compression_report(
        ...
        report = {
            ...
            "total_changes_applied": sum(r.get("total_changes", 0) for r in results),
            "files_modified": len(
                [r for r in results if r.get("content_modified", False)]
            ),
...
def main():
    ...
    report_path = (
        semantic_dir
        / f"semantic_compression_{timestamp.strftime('%Y%m%d_%H%M%S')}.json"
    )
