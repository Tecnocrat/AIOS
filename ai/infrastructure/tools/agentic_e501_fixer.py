...
try:
    ...
    logging.warning("Hierarchical pipeline not available, using facade fallback")
...
class AgenticE501Fixer:
    ...
    def __init__(self, use_hierarchy: bool = True):
        ...
        if use_hierarchy and HIERARCHICAL_AVAILABLE:
            ...
            logging.info(
                "Hierarchical pipeline initialized " "(Ollama→Gemini→DeepSeek)"
            )
        elif use_hierarchy and not HIERARCHICAL_AVAILABLE:
            logging.warning(
                "Hierarchical pipeline requested but unavailable, " "using facade"
            )
...
Return the fixed code as plain text, one line per line."""
    ...
    async def fix_line(self, line: str, line_number: int, file_path: str) -> FixResult:
        """
        ...
        if agents_available:
            try:
                result = await self.fix_line_with_agent(line, line_number, file_path)
                if result.success:
                    self.stats["agent_fixes"] += 1
                return result
            except Exception as e:
                logger.warning(f"Agent conclave failed for line {line_number}: {e}")
    ...
    async def fix_file(self, file_path: str, dry_run: bool = True):
        ...
        for i, line in enumerate(lines):
            ...
            if len(line_content) > MAX_LINE_LENGTH:
                try:
                    result = await self.fix_line(line_content, i + 1, file_path)
                    ...
                    fixed_lines.append(line)
                    fix_results.append({"line_number": i + 1, "error": str(e)})
            else:
                fixed_lines.append(line)
...
async def main():
    ...
    parser.add_argument(
        "--fix", action="store_true", help="Apply fixes to files (default: dry-run)"
    )
    parser.add_argument(
    ...
    if not agents_available:
        logger.warning("No AI agents available. Will use basic pattern-based fixing.")
