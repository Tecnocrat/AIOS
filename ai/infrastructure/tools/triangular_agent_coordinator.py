...
from aios_gemma_bridge import AIOSGemmaBridge, RouteTarget, SignalType
...
class TriangularCoordinator:
    ...
    def __init__(self, max_retries: int = 2, require_validation: bool = True):
        """
    ...
    async def fix_e501(
        ...
        if route_to == RouteTarget.GEMMA and gemma_result.signal:
            quick_fix = await self.gemma.quick_fix(
                line, gemma_result.signal.signal_type
            )
            if quick_fix:
        ...
        if not mistral_response.success or not mistral_response.content:
            ...
            return await self._fallback_result(
                line, gemma_signal, tier_path, start_time, "Mistral generation failed"
            )
        ...
        while retries <= self.max_retries:
            context = GeminiContext(
                ...
                upstream_analysis=UpstreamAnalysis(
                    gemma_signal=gemma_signal, mistral_output=current_fix
                ),
...
async def main():
    ...
    async with TriangularCoordinator() as coord:
        ...
        result = await coord.fix_e501(
            line=test_line, file_path="test.py", line_number=1
        )
