...
class ImportPathUpdater:
    ...
    def detect_old_imports(self, file_path: Path) -> List[Tuple[int, str, str, str]]:
        ...
        try:
            ...
            for line_num, line in enumerate(lines, 1):
                ...
                match3 = re.match(
                    r"^from\s+runtime\.tools(?:\.(\w+))?\s+import\s+", line
                )
                if match3:
                    ...
                    imported_tools = [
                        t.strip().split()[0] for t in import_part.split(",")
                    ]
            ...
            self.errors.append(
                {"file": str(file_path), "error": f"Error reading file: {e}"}
            )
    ...
    def generate_new_import(self, old_line: str, tool_name: str, category: str) -> str:
        ...
        if "from runtime.tools import" in old_line:
            new_line = old_line.replace(
                "from runtime.tools import", f"from ai.tools.{category} import"
            )
            return new_line
    ...
    def generate_phase2c_import(self, old_line: str, import_type: str) -> str:
        ...
        if import_type == "from_core_module":
            ...
            new_line = re.sub(
                r"\bimport core\.", "import computational_layer.", old_line
            )

        return new_line

    def update_file(
        self, file_path: Path, detections: List[Tuple[int, str, str, str]]
    ) -> bool:
        """
        ...
        try:
            ...
            self.errors.append(
                {"file": str(file_path), "error": f"Error updating file: {e}"}
            )
            return False

    def update_file_phase2c(
        self, file_path: Path, detections: List[Tuple[int, str, str]]
    ) -> bool:
        """
    ...
    def process_workspace(self, phase2c_only: bool = False):
        ...
        """
        migration_type = (
            "Phase 2C (core -> computational_layer)"
            if phase2c_only
            else "Runtime Intelligence (runtime.tools -> ai.tools)"
        )
    ...
    def print_report(self, phase2c_mode: bool = False):
        ...
        print(
            f"Mode: {'DRY RUN (preview only)' if self.dry_run else 'EXECUTE (changes applied)'}"
        )
        ...
        if self.changes:
            if phase2c_mode:
                ...
                for change in self.changes:
                    ...
                    print(f"  Line: {change['line']}")
                    print(
                        f"  Tool: {change.get('tool', 'unknown')} → {change.get('category', 'unknown')}"
                    )
                    print(f"  Old:  {change['old']}")
                    print(f"  New:  {change['new']}")
                    print()
        ...
        if self.dry_run:
            ...
            print(
                "[EXECUTE] Changes have been applied. Backup files created with .backup extension."
            )
            print()
            print("NEXT STEPS:")
            print(
                "  1. Run validation: python ai/tools/update_import_paths.py --validate"
            )
    ...
    def validate_imports(self):
        ...
        for tool_name, category in TOOL_CATEGORIES.items():
            try:
                ...
                validation_errors.append(
                    {"tool": tool_name, "category": category, "error": str(e)}
                )
        ...
        if validation_errors:
            ...
            print(
                "Note: Some errors may be expected (missing dependencies, syntax errors in tool code)"
            )
        else:
...
Examples:
  ...
  python update_import_paths.py --validate
    ...
    parser.add_argument(
        "--validate", action="store_true", help="Validate that imports can be resolved"
    )
