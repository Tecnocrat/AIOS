...
class AIOSCppAnalyzer:
    ...
    def _integration_assembler_pass(self, content: str) -> Dict[str, Any]:
        ...
        if "std::cout" in content and "logging" not in content.lower():
            integration_issues.append(
                "Consider using AIOS logging instead of std::cout"
            )
    ...
    def _synthesize_analysis(
        ...
        dendritic_analysis: Dict,
    ) -> CppAnalysisResult:
        """Synthesize all analysis passes into comprehensive result"""

        # Calculate composite scores
        complexity_score = (
            10.0 - env_analysis["complexity_indicators"]["cyclomatic_complexity"]
        )
        maintainability_score = (
            env_analysis["coherence_score"]
            + integration_analysis["integration_score"] / 10.0
        )
        performance_score = len(dendritic_analysis["evolution_patterns"]) * 1.5
        security_score = (
            8.0 if consciousness_analysis["consciousness_level"] > 0.5 else 6.0
        )
    ...
    def _calculate_code_coherence(self, content: str) -> float:
        ...
        comment_ratio = len([line for line in lines if line.startswith("//")]) / max(
            len(lines), 1
        )
    ...
    for file_path in core_files:
        if os.path.exists(file_path):
            ...
            if result.architectural_patterns:
                print(
                    f"    Architectural Patterns: {', '.join(result.architectural_patterns)}"
                )
            ...
            if result.enhancement_suggestions:
                print(
                    f"    Suggestions: {', '.join(result.enhancement_suggestions[:2])}"
                )
