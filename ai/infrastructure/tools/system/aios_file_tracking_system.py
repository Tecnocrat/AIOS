...
class FileTracker:
    ...
    def __init__(self, workspace_root: str):
        ...
        self.tracking_file = (
            self.workspace_root / "runtime" / "file_tracking_database.json"
        )
        ...
        serializable_data = {
            ...
            "metadata": {
                ...
                "total_dependencies": sum(
                    len(deps)
                    for deps in self.tracking_data["dependency_graph"].values()
                ),
    ...
    def determine_file_purpose(
        self, file_path: Path, imports: List[str], exports: List[str]
    ) -> str:
    ...
    def full_workspace_scan(self) -> Dict[str, Any]:
        ...
        for file_path in filtered_files:
            ...
            if record:
                ...
                if rel_path in self.tracking_data["files"]:
                    ...
                    old_hash = (
                        old_record_data.get("file_hash")
                        if isinstance(old_record_data, dict)
                        else old_record_data.file_hash
                    )
        ...
        scan_results = {
            ...
            "total_dependencies": sum(
                len(deps) for deps in self.tracking_data["dependency_graph"].values()
            ),
        }
    ...
    def track_structural_change(
        self, change_type: str, old_path: Optional[str], new_path: Optional[str]
    ):
...
"""
        ...
        recent_structural = self.tracking_data["structural_changes"][
            -10:
        ]  # Last 10 changes
        for change in recent_structural:
            ...
            report_content += (
                f"- **{change_type}**: {old_path} → {new_path} ({timestamp})\n"
            )
        ...
        for change in recent_deps:
            ...
            report_content += (
                f"- **{change_type}**: {source_file} → {target_file} ({timestamp})\n"
            )
