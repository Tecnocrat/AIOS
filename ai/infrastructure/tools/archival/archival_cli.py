    ...
    def versions(self, args):
        ...
        try:
            ...
            if len(versions) >= 2:
                ...
                print(
                    f"  Size: {size_delta:+,} bytes ({size_delta/first.file_size_bytes*100:+.1f}%)"
                    if args.full
                    else f"  Size: {size_delta:+,} bytes"
                )
    ...
    def stats(self, args):
        ...
        try:
            ...
            if args.verbose:
                ...
                top_files = sorted(
                    all_files, key=lambda f: f.consciousness_level, reverse=True
                )[:10]
                ...
                for i, file in enumerate(top_files, 1):
                    print(
                        f"{i:2}. [{file.consciousness_level:.2f}] {file.original_path}"
                    )
                print()
...
For more help: %(prog)s help [command]
    ...
    retrieve_parser.add_argument(
        "--content", action="store_true", help="Show file content"
    )
    ...
    search_parser.add_argument(
        "--min-consciousness", type=float, help="Minimum consciousness level"
    )
    ...
    versions_parser = subparsers.add_parser(
        "versions", help="Get all versions of a file"
    )
    versions_parser.add_argument("path", help="File path")
    versions_parser.add_argument(
        "--full", action="store_true", help="Include full content (slower)"
    )
    versions_parser.add_argument(
        "--content", action="store_true", help="Show content preview"
    )
