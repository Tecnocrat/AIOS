...
class RobustPythonEnvironmentManager:
    ...
    def discover_python_installations(self) -> List[PythonEnvironment]:
        ...
        self.logger.info(
            f"Discovered {len(verified_installations)} valid Python installations"
        )
    ...
    def refresh_environments(self) -> int:
        ...
        self.logger.info(
            f"Refresh complete: {healthy_count}/{len(self.environments)} environments healthy"
        )
    ...
    def create_virtual_environment(
        self, name: str, base_python: str = None, location: str = None
    ) -> Optional[str]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                try:
                    ...
                    if version_result.returncode == 0:
                        env.version = version_result.stdout.strip().replace(
                            "Python ", ""
                        )
                except Exception:
                    pass
    ...
    def export_environment_config(self, output_file: str = None) -> str:
        ...
        if output_file is None:
            output_file = os.path.join(
                self.config_dir,
                f"python_env_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
            )
        ...
        export_data = {
            ...
            "aios_version": "1.0.0",
            "environments": {
                env_id: asdict(env) for env_id, env in self.environments.items()
            },
        }
        ...
        try:
            ...
            for env_id, env_data in import_data.get("environments", {}).items():
                ...
                for existing_env in self.environments.values():
                    if (
                        os.path.normpath(existing_env.python_path).lower()
                        == os.path.normpath(env_data["python_path"]).lower()
                    ):
                        existing = True
                        break
            ...
            self._save_environments()
            self.logger.info(
                f"Imported {imported_count} environments from {import_file}"
            )
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        if health_report["healthy_environments"] == 0:
            health_report["recommendations"].append(
                "No healthy Python environments found. Run discovery or install Python."
            )
        ...
        if health_report["missing_environments"] > 0:
            health_report["recommendations"].append(
                f"{health_report['missing_environments']} environments are missing. Consider cleanup."
            )
