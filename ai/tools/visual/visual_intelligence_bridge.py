...
class AIOSVisualIntelligence:
    ...
    def _extract_screenshot_activity(self) -> Dict:
        ...
        return {
            ...
            "activity_level": (
                "high"
                if len(recent_screenshots) > 10
                else "moderate" if len(recent_screenshots) > 0 else "low"
            ),
        }
    ...
    def _generate_extraction_summary(
        ...
        if session:
            summary_parts.append(
                f"Session Data: Extracted {session.get('SessionId', 'Unknown')[:8]}..."
            )
        ...
        if frame:
            summary_parts.append(
                f"Frame Data: Frame {frame.get('FrameNumber', 0)} extracted"
            )
            summary_parts.append(
                f"Resolution: {frame.get('Resolution', {}).get('Width', 0)}x{frame.get('Resolution', {}).get('Height', 0)}"
            )
        else:
        ...
        if activity_status == "active":
            summary_parts.append(
                f"Activity Data: {activity['total_screenshots']} screenshots, {activity['total_size_mb']:.1f} MB"
            )
        else:
