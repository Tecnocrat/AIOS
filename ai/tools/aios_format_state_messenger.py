...
class AIOSFormatStateMessenger:
    ...
    def _save_state(self) -> None:
        ...
        try:
            ...
            data = {
                ...
                "last_heartbeat": (
                    self.state.last_heartbeat.isoformat() if self.state.last_heartbeat else None
                ),
    ...
    def report_format_event(
        self, file_path: str, action: FormatAction, changes_detected: bool = False
    ) -> bool:
        """
...
def handle_format_save(file_path: str, action: str = "format") -> None:
    ...
    action_enum = (
        FormatAction(action) if action in [a.value for a in FormatAction] else FormatAction.FORMAT
    )
...
def main():
    ...
    parser.add_argument(
        "--action", "-a", choices=["format", "save", "check", "sync"], default="format"
    )
