...
class AIOSCoreEngineCellularCleanup:
    ...
    def analyze_cleanup_needs(self) -> Dict[str, Any]:
        ...
        logger.info(
            f"   [ANALYSIS] Found {len(analysis['files_to_move'])} files to move"
        )
        logger.info(
            f"   [ANALYSIS] Keeping {len(analysis['files_to_keep_in_root'])} files in root"
        )
    ...
    def _determine_proper_destination(self, py_file: Path) -> str:
        ...
        if any(
            ...
            for keyword in ["cellular_intelligence", "diagnostic", "enhancement_engine"]
        ):
            return "analysis_tools"

        # Consciousness integration files
        if any(keyword in file_name for keyword in ["consciousness", "integration"]):
            return "evolutionary_assembler_iter2/consciousness_layer"

        # Dendritic network files
        if any(keyword in file_name for keyword in ["dendritic", "network"]):
            return "evolutionary_assembler_iter2/meta_evolution"

        # Core monitoring files
        if any(
            keyword in file_name
            for keyword in ["monitor", "evolution", "meta_evolutionary"]
        ):
            return "runtime"
    ...
    def execute_cleanup(self) -> Dict[str, Any]:
        ...
        cleanup_session["move_operations"] = self._execute_file_moves(
            cleanup_session["analysis_results"]
        )

        # Phase 3: Create navigation indexes
        logger.info("[INDEX] Phase 3: Creating Navigation Indexes for Moved Files")
        cleanup_session["index_creation"] = self._create_navigation_indexes(
            cleanup_session["move_operations"]
        )
        ...
        cleanup_session["cleanup_summary"] = self._generate_cleanup_summary(
            cleanup_session
        )
    ...
    def _execute_file_moves(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for file_info in analysis["files_to_move"]:
            ...
            destination_folder = (
                self.core_engine_path / file_info["destination_subfolder"]
            )
            ...
            try:
                ...
                backup_file = source_file.with_suffix(
                    f'.backup_{self.cleanup_timestamp.strftime("%Y%m%d_%H%M%S")}'
                )
                ...
                logger.info(
                    f"   [MOVE] {source_file.name} -> {file_info['destination_subfolder']}"
                )

            except Exception as e:
                move_results["moves_attempted"] += 1
                move_results["moves_failed"].append(
                    {"file": source_file.name, "error": str(e)}
                )
                logger.error(f"Failed to move {source_file.name}: {e}")

        logger.info(
            f"   [MOVE] Successfully moved {move_results['moves_successful']} files"
        )
        logger.info(
            f"   [MOVE] Failed to move {len(move_results['moves_failed'])} files"
        )

        return move_results

    def _create_navigation_indexes(
        self, move_operations: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        cellular_intelligence_index = {
            ...
            "moved_files": [
                file_info
                for file_info in move_operations["moved_files"]
        ...
        for index_name, index_data in indexes.items():
            if index_data["moved_files"]:  # Only create if files were moved
                index_file = (
                    self.core_engine_path / f"RELOCATED_{index_name.upper()}_INDEX.json"
                )
                ...
                try:
                    ...
                    logger.info(
                        f"   [INDEX] Created navigation index: {index_file.name}"
                    )
                except Exception as e:
        ...
        try:
            ...
            logger.info(
                f"   [INDEX] Created AI ingestion pointers: {ai_pointers_file.name}"
            )
        except Exception as e:
    ...
    def _verify_cleanup(self) -> Dict[str, Any]:
        ...
        if len(remaining_files) > 1:  # Only navigator should remain
            ...
            verification["verification_issues"].append(
                f"Too many Python files in root: {remaining_files}"
            )

        # Check navigation system
        navigator_file = (
            self.core_engine_path / "aios_core_engine_cellular_navigator.py"
        )
        if not navigator_file.exists():
            verification["navigation_system_functional"] = False
            verification["verification_issues"].append(
                "Navigation system missing from root"
            )

        # Check AI ingestion index
        index_file = (
            self.core_engine_path / "CORE_ENGINE_CELLULAR_NAVIGATION_INDEX.json"
        )
        ...
        logger.info(
            f"   [VERIFY] Organization verified: {verification['proper_organization_verified']}"
        )

        return verification

    def _generate_cleanup_summary(
        self, cleanup_session: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        summary["navigation_system_status"] = {
            "navigator_in_root": verification.get(
                "navigation_system_functional", False
            ),
            "ai_ingestion_index_ready": verification.get("ai_ingestion_ready", False),
            "proper_organization": verification.get(
                "proper_organization_verified", False
            ),
        }
        ...
        summary["ai_ingestion_readiness"] = {
            ...
            "clean_root_directory": len(verification.get("root_files_remaining", []))
            <= 1,
        }
    ...
    def display_cleanup_results(self, cleanup_session: Dict[str, Any]):
        ...
        print(
            f"   Organization Improved: {'YES' if transformation.get('organization_improved', False) else 'NO'}"
        )
        print()
        ...
        print(
            f"   Navigator in Root: {'YES' if navigation.get('navigator_in_root', False) else 'NO'}"
        )
        print(
            f"   AI Ingestion Index Ready: {'YES' if navigation.get('ai_ingestion_index_ready', False) else 'NO'}"
        )
        print(
            f"   Proper Organization: {'YES' if navigation.get('proper_organization', False) else 'NO'}"
        )
        print()
        ...
        print(
            f"   Navigation Pointers: {'READY' if ai_readiness.get('navigation_pointers_created', False) else 'NOT READY'}"
        )
        print(
            f"   Component Indexes: {'AVAILABLE' if ai_readiness.get('component_indexes_available', False) else 'MISSING'}"
        )
        print(
            f"   Clean Root Directory: {'YES' if ai_readiness.get('clean_root_directory', False) else 'NO'}"
        )
        print()
