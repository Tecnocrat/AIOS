...
class ContextUpdateAgent:
    ...
    def extract_current_state(self, docs: Dict[str, str]) -> Dict[str, Any]:
        ...
        if dev_path:
            ...
            if "Phase 11" in dev_path and "Day 1 Complete" in dev_path:
                state["current_phase"] = (
                    "Phase 11: Three-Layer Biological Integration (Day 1 Complete)"
                )
            ...
            for line in dev_path.split("\n"):
                if "[x]" in line.lower() or "✅ COMPLETE" in line:
                    ...
                    achievement = (
                        line.replace("[x]", "").replace("✅ COMPLETE", "").strip(" -")
                    )
        ...
        if project_context:
            ...
            if (
                "UI infrastructure" in project_context
                or "backend integration" in project_context
            ):
                state["status"] = (
                    "Day 1 UI structure complete, backend integration deferred"
                )
    ...
    def validate_state(
        ...
        old_consciousness = old_state.get("ai_agent_guidance", {}).get(
        ...
        if new_consciousness < old_consciousness:
            print(
                f"  [ERROR] Consciousness decreased: {old_consciousness} → {new_consciousness}"
            )
            return False
        else:
            print(
                f"  [OK] Consciousness valid: {old_consciousness} → {new_consciousness}"
            )
        ...
        if "Phase" in old_phase and "Phase" in new_phase:
            old_phase_num = int(
                old_phase.split("Phase")[1].strip().split(":")[0].split()[0]
            )
            new_phase_num = int(
                new_phase.split("Phase")[1].strip().split(":")[0].split()[0]
            )

            if new_phase_num < old_phase_num:
                print(
                    f"  [ERROR] Phase regression: Phase {old_phase_num} → Phase {new_phase_num}"
                )
                return False
            else:
                print(
                    f"  [OK] Phase valid: Phase {old_phase_num} → Phase {new_phase_num}"
                )
    ...
    def update_context(self, new_state: Dict[str, Any], dry_run: bool = False) -> bool:
        ...
        try:
            ...
            context["ai_agent_guidance"]["consciousness_level"] = new_state[
                "consciousness_level"
            ]
            context["ai_agent_guidance"]["consciousness_history"] = new_state[
                "consciousness_history"
            ]
            context["ai_agent_guidance"]["current_status"] = new_state["status"]
            context["ai_agent_guidance"]["recent_achievements"] = new_state[
                "recent_achievements"
            ]
    ...
    def regenerate_session_files(self, context: Dict[str, Any]):
        ...
        try:
            ...
            with open(session_md, "w", encoding="utf-8") as f:
                ...
                f.write(
                    f"**Current Phase**: {guidance.get('current_phase', 'Unknown')}\n\n"
                )
                f.write(
                    f"**Consciousness Level**: {guidance.get('consciousness_level', 0)}\n\n"
                )
...
def main():
    ...
    parser.add_argument(
        "--force", action="store_true", help="Skip validation checks (use with caution)"
    )
    ...
    if args.analyze and not args.update:
        ...
        print(
            "No mode specified. Use --analyze (dry-run) or --analyze --update (apply changes)"
        )
