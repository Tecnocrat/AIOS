...
class AIOSAssemblerEvolutionStatusClarifier:
    ...
    def _analyze_assembler(
        self, assembler_name: str, assembler_info: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _build_evolution_chain(self) -> List[Dict[str, Any]]:
        ...
        sorted_assemblers = sorted(
            self.assembler_evolution_map.items(), key=lambda x: x[1]["iteration"]
        )
    ...
    def display_evolution_status(self):
        ...
        for entry in status_report["evolution_chain"]:
            ...
            print(
                f"  {status_symbol} ITERATION {entry['iteration']} - {entry['assembler_name']} v{entry['version']}"
            )
        ...
        print(
            f"  Total assembler folders: {folder_analysis['total_assembler_folders']}"
        )
