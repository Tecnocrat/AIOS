...
class ParadigmExtractionEngine:
    ...
    def extract_from_library(self, library_name: str) -> List[ProgrammingParadigm]:
        ...
        if not json_files:
            ...
            logger.info(
                f"💡 Run library ingestion first: LibraryIngestionProtocol().ingest_library('{library_name}')"
            )
            return []
    ...
    def _extract_from_file(self, json_file: Path):
        ...
        try:
            ...
            if "api_elements" in knowledge:
                ...
                functions = [
                    e
                    for e in knowledge["api_elements"]
                ...
                classes = [
                    e for e in knowledge["api_elements"] if e["element_type"] == "class"
                ]
...
def main():
    ...
    output_path = (
        Path(__file__).parent.parent.parent
        / "information_storage"
        / f"{library_name}_paradigms.json"
    )
