...
<<<<<<< HEAD
        self.cache_dir = cache_dir or (
            AIOS_ROOT / "docs" / "libraries" / "cpp_stl" / "raw_documentation"
        )
...
<<<<<<< HEAD
                ...
                logger.info(
                    f"Resumed crawl: {len(self.visited_urls)} visited, {len(self.pending_urls)} pending"
                )
...
<<<<<<< HEAD
        ...
        # Class documentation
        if "class" in url_lower or soup.find(
            "h1", string=re.compile(r"class|struct", re.I)
        ):
            return "class"

        # Function documentation
        if any(
            keyword in url_lower for keyword in ["function", "algorithm", "numeric"]
        ):
            return "function"
        ...
        if any(
            ...
            for keyword in ["overview", "reference", "introduction"]
        ):
            return "overview"
...
<<<<<<< HEAD
    ...
    async def fetch_page(
        self, session: aiohttp.ClientSession, url: str, use_cache: bool = True
    ) -> Optional[str]:
        """
...
<<<<<<< HEAD
    ...
    async def crawl_page(self, session: aiohttp.ClientSession, url: str) -> Dict:
        """
...
<<<<<<< HEAD
        ...
        async with aiohttp.ClientSession(
            headers={
                "User-Agent": "AIOS-STL-Crawler/1.0 (Educational; https://github.com/Tecnocrat/AIOS)"
            }
        ) as session:
...
<<<<<<< HEAD
    ...
    def show_progress(stats):
        print(
            f"  Crawled: {stats['pages_crawled']}, STL: {stats['stl_pages_found']}",
            end="\r",
        )
