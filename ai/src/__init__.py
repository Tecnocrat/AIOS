...
class AICore:
    ...
    This class serves as the central hub for all AI operations in AIOS,
    managing NLP, prediction, automation, and learning subsystems.
    """
    ...
    def __init__(self, config_path: str = "config/ai-models.json"):
        ...
        self.prediction_manager = PredictionManager(self.config.get("models", {}).get("prediction", {}))
        self.automation_manager = AutomationManager(self.config.get("models", {}).get("automation", {}))
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to start AI Core services: {e}")
            return False

    async def stop(self):
        """Stop all AI Core services."""
    ...
    async def process_natural_language(self, text: str, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            # Process through NLP manager
            nlp_result = await self.nlp_manager.process(text, context)
            
            # Determine intent and route to appropriate subsystem
            intent = nlp_result.get("intent", "unknown")
            
            if intent == "automation":
                automation_result = await self.automation_manager.process_task(nlp_result)
                return {
                    "status": "success",
                    "intent": intent,
    ...
    async def get_system_prediction(self, metric: str, horizon: int = 60) -> Dict[str, Any]:
        """
    ...
    def get_status(self) -> Dict[str, Any]:
        """
        Get current status of AI Core and all subsystems.
...
def get_ai_core() -> AICore:
    ...
    Returns:
        AICore: The global AI Core instance
    """
