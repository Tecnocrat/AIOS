...
def execute_multiple_iterations(iterations=3):
    ...
    print(
        f"   • Cumulative Improvement: +{summary['cumulative_coherence_improvement']:.3f}"
    )
    print(
        f"   • Average Improvement/Pass: +{summary['average_improvement_per_pass']:.3f}"
    )

    print(f"\n Optimization Progression:")
    for prog in session_report["optimization_progression"]:
        print(
            f"   Pass {prog['pass_number']}: {prog['focus_area']} (+{prog['coherence_improvement']:.3f})"
        )
    ...
    try:
        with open(results_file, "w") as f:
            json.dump(
                {"session_report": session_report, "all_iterations": all_results},
                f,
                indent=2,
                default=str,
            )
