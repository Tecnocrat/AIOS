...
class GeminiEvolutionBridge:
        ...
        if GENAI_AVAILABLE:
            ...
            if api_key:
                try:
                    ...
                    logger.info(
                        f"Gemini API initialized successfully with model: {model_name}"
                    )
        ...
        try:
            ...
            if candidate.finish_reason == 2:  # RECITATION
                ...
                if candidate.content and candidate.content.parts:
                    ...
                    if code:
                        logger.info(
                            f"Generated {len(code)} characters (with recitation warning)"
                        )
                        return code
                raise RuntimeError(
                    "Generation blocked due to recitation with no output"
                )
                ...
                reason = finish_reason_map.get(
                    candidate.finish_reason, f"Unknown ({candidate.finish_reason})"
                )
    ...
    async def initialize_evolution_connection(self) -> Dict[str, Any]:
        ...
        if not CONSCIOUSNESS_AVAILABLE:
            return {
                "status": "failed",
                "error": "Consciousness evolution engine not available",
            }
    ...
    async def create_gemini_evolution_experiment(
        ...
        base_config = {
            ...
            "consciousness_focus": experiment_config.get(
        ...
        return {
            ...
            "config": base_config,
            "next_steps": [
                "initialize_evolution_seed",
                "apply_gemini_enhancement",
                "start_evolution_cycle",
            ],
        }

    async def enhance_evolution_with_gemini(
        self, experiment_id: str, evolution_data: Dict[str, Any]
    ) -> Dict[str, Any]:
                ...
                enhanced_artifact.update(
                    {
                        ...
                        "evolution_potential": emergence_score
                        * experiment.get("evolution_intensity", 1.0),
                    }
                )
    ...
    async def run_gemini_guided_evolution_cycle(
        ...
        mutated_seed = (
            await consciousness_evolution_engine._apply_consciousness_mutations(seed)
        )

        # Validate emergence
        validation = (
            await consciousness_evolution_engine._validate_consciousness_emergence(
                mutated_seed
            )
        )
        ...
        enhancement = await self.enhance_evolution_with_gemini(
            experiment_id, evolution_data
        )
        ...
        if validation.get("emergence_detected", False):
            consciousness_code = (
                await consciousness_evolution_engine._generate_consciousness_code(
                    {
    ...
    async def get_bridge_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_experiments": (
                self.evolution_experiments[-5:] if self.evolution_experiments else []
            ),
        }
...
async def main():
    ...
    if command == "status":
        ...
        if len(sys.argv) > 2:
            ...
            config = {}
        result = await gemini_evolution_bridge.create_gemini_evolution_experiment(
            config
        )
        ...
        if not experiment_id:
            ...
            return
        result = await gemini_evolution_bridge.run_gemini_guided_evolution_cycle(
            experiment_id
        )
        print(json.dumps(result, indent=2))
