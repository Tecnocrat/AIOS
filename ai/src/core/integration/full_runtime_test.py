...
class FullRuntimeTestSuite:
    ...
    async def _test_context_persistence_system(self):
        ...
        try:
            ...
            print(f"   📊 Context health score: {health_result.get('health_score', 0):.3f}")
            ...
            print(f"   📸 Snapshot created: {snapshot.get('snapshot_id', 'unknown')}")
            ...
            print(f"   🔄 Recovery status: {recovery_result.get('status', 'unknown')}")
    ...
    async def _test_holographic_synchronization(self):
        ...
        try:
            ...
            print(f"   🔄 Sync initialization: {sync_status.get('status', 'unknown')}")
    ...
    async def _test_debug_integration(self):
        ...
        try:
            ...
            print(f"   🎯 Debug session: {session.get('session_id', 'unknown')}")

            # Test context snapshot during debug
            print("   🔍 Testing debug context snapshot...")
            debug_snapshot = session.get('debug_snapshot', {})
            print(f"   📸 Debug snapshot: {debug_snapshot.get('snapshot_id', 'unknown')}")
            ...
            print(f"   ✅ Debug completion: {completion_result.get('status', 'unknown')}")
    ...
    async def _generate_comprehensive_report(self):
        ...
        if 'context_persistence' in self.test_results:
            ...
            if isinstance(ctx_result, dict) and 'health_score' in ctx_result:
                ...
                print(f"💚 Context Health: {health_status} ({health_score:.3f})")
        ...
        if 'holographic_sync' in self.test_results:
            ...
            if isinstance(sync_result, dict) and 'holographic_coherence' in sync_result:
                ...
                print(f"🌟 Holographic Coherence: {coherence_status} ({coherence:.3f})")
