...
def test_aios_integration():
    ...
    print(f"  Recovery applied: {health_report['aios_integration']['recovery_applied']}")
    ...
    if snapshot['active_environment']:
        print(f"Active environment in snapshot: {snapshot['active_environment']['name']}")
...
def test_os_reinstall_preparation():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if backup_file and os.path.exists(backup_file):
            ...
            print(f"  - Environment snapshot: {len(backup_data['environment_snapshot']['environments'])} environments")
            print(f"  - Recovery instructions: {len(backup_data['recovery_instructions'])} steps")
