#!/usr/bin/env python3
"""
AIOS Python Environment Manager
Robust, OS-resilient Python interpreter and PATH management system

This module provides:
- Python interpreter detection and validation
- PATH management and recovery
- Environment diagnostics and self-healing
- OS reinstall survival strategies
- Integration with AIOS/AINLP for automatic issue resolution
"""
...
class PythonEnvironment:
    ...
    version: str
    architecture: str
    environment_type: str  # system, venv, conda, portable, msys2
    pip_available: bool
    packages: List[str]
    health_status: str
    issues: List[str]
...
class PythonEnvironmentManager:
    ...
    def __init__(self, aios_root: str = None):
        self.aios_root = (aios_root or
                         os.path.dirname(os.path.dirname(
                             os.path.dirname(os.path.abspath(__file__)))))
        self.config_path = os.path.join(self.aios_root, "config",
                                       "python-environment.json")
        self.backup_config_path = os.path.join(self.aios_root, "config",
    ...
    def _detect_windows_python_installations(self) -> List[PythonEnvironment]:
        ...
        try:
            ...
            for hive in [winreg.HKEY_LOCAL_MACHINE, winreg.HKEY_CURRENT_USER]:
                try:
                    python_key = winreg.OpenKey(hive,
    ...
    def _detect_common_path_installations(self) -> List[PythonEnvironment]:
        ...
        common_paths = [
            ...
            r"C:\msys64\mingw64\bin\python.exe",  # MSYS2
            r"C:\msys64\usr\bin\python.exe",      # MSYS2
        ]
    ...
    def _analyze_python_executable(self, executable_path: str) -> Optional[PythonEnvironment]:
        ...
        try:
            ...
            # Get version info
            result = subprocess.run([executable_path, "-c",
                                   "import sys; print(f'{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}'); print(sys.platform); print(sys.maxsize > 2**32)"],
                                  capture_output=True, text=True, timeout=10)
            ...
            pip_result = subprocess.run([executable_path, "-m", "pip", "--version"],
                                      capture_output=True, text=True, timeout=10)
            ...
            if pip_available:
                try:
                    pkg_result = subprocess.run([executable_path, "-m", "pip", "list", "--format=freeze"],
                                              capture_output=True, text=True, timeout=30)
                    if pkg_result.returncode == 0:
                        packages = [line.strip() for line in pkg_result.stdout.split('\n') if line.strip()]
                except Exception:
                    pass
            ...
            # Health assessment
            health_status, issues, recommendations = self._assess_environment_health(
    ...
    def _assess_environment_health(self, executable_path: str, version: str,
                                 ...
                                 env_type: str) -> Tuple[str, List[str], List[str]]:
        ...
        if env_type == "msys2":
            ...
            recommendations.append("Consider using standard Windows Python distribution")
        ...
        installed_package_names = [pkg.split('==')[0].split('>=')[0].split('<=')[0]
                                 for pkg in packages]
        ...
        if missing_packages:
            issues.append(f"Missing AIOS required packages: {', '.join(missing_packages)}")
    ...
    def get_recommended_environment(self) -> Optional[PythonEnvironment]:
        ...
        def score_environment(env: PythonEnvironment) -> int:
            ...
            # Health status
            health_scores = {"excellent": 100, "good": 80, "fair": 50, "poor": 20}
            score += health_scores.get(env.health_status, 0)

            # Environment type preference
            type_scores = {"system": 20, "user": 15, "conda": 10, "venv": 8, "msys2": 5, "unknown": 0}
    ...
    def create_portable_python_environment(self) -> bool:
        ...
        try:
            ...
            zip_path = os.path.join(self.portable_python_path, "python-embed.zip")
            ...
            pth_file = os.path.join(self.portable_python_path, f"python{python_version.replace('.', '')[:2]}._pth")
            ...
            get_pip_path = os.path.join(self.portable_python_path, "get-pip.py")
    ...
    def install_aios_requirements(self, python_executable: str) -> bool:
        ...
        try:
            requirements_file = os.path.join(self.aios_root, "ai", "requirements.txt")
            ...
            result = subprocess.run([python_executable, "-m", "pip", "install", "-r", requirements_file],
    ...
    def save_environment_config(self, environment: PythonEnvironment):
        ...
        config = {
            ...
            "backup_environments": [env.__dict__ for env in self.detect_all_python_installations()[:5]]
        }
    ...
    def auto_heal_environment(self) -> bool:
        ...
        if config and "selected_environment" in config:
            ...
            if os.path.exists(saved_env["executable_path"]):
                current_env = self._analyze_python_executable(saved_env["executable_path"])
                if current_env and current_env.health_status in ["excellent", "good"]:
                    print(f"✅ Saved environment still healthy: {saved_env['executable_path']}")
                    return True
        ...
        if not recommended_env:
            print("❌ No suitable Python environment found. Creating portable environment...")
            if self.create_portable_python_environment():
                portable_python = os.path.join(self.portable_python_path, "python.exe")
                recommended_env = self._analyze_python_executable(portable_python)
            else:
                print("❌ Failed to create portable environment")
                return False

        if recommended_env:
            print(f"✅ Selected Python environment: {recommended_env.executable_path}")
    ...
    def get_environment_command_prefix(self) -> str:
        """Get the command prefix for running Python in the current environment"""
        config = self.load_environment_config()
    ...
    def diagnose_issues(self) -> Dict[str, Any]:
        ...
        environments = self.detect_all_python_installations()
        diagnostics["python_installations"] = [env.__dict__ for env in environments]
        ...
        if not environments:
            ...
            diagnostics["recommendations"].append("Install Python from python.org")
            diagnostics["aios_compatibility"] = "incompatible"
        else:
            recommended = self.get_recommended_environment()
            if recommended and recommended.health_status in ["excellent", "good"]:
                diagnostics["aios_compatibility"] = "compatible"
            else:
                diagnostics["aios_compatibility"] = "needs_setup"
                diagnostics["recommendations"].append("Run auto-heal to configure optimal environment")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="AIOS Python Environment Manager")
    parser.add_argument("--diagnose", action="store_true", help="Run environment diagnostics")
    parser.add_argument("--heal", action="store_true", help="Auto-heal environment issues")
    parser.add_argument("--list", action="store_true", help="List all Python installations")
    parser.add_argument("--portable", action="store_true", help="Create portable Python environment")
    ...
    if args.diagnose:
        ...
        print("✅ Environment healed successfully!" if success else "❌ Healing failed")
        ...
        print("✅ Portable environment created!" if success else "❌ Creation failed")
