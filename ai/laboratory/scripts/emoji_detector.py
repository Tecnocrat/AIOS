...
class EmojiDetector:
    ...
    def scan_directory(self, directory: Path, extensions: List[str] = None) -> List[EmojiMatch]:
        """Scan directory for emoji usage"""
        if extensions is None:
            extensions = ['.md', '.py', '.ps1', '.txt', '.json', '.yaml', '.yml']
    ...
    def generate_report(self, matches: List[EmojiMatch], output_format: str = 'console') -> str:
    ...
    def _console_report(self, matches: List[EmojiMatch]) -> str:
        ...
        for file_path, file_matches in by_file.items():
            ...
            for match in file_matches:
                report.append(f"  Line {match.line_number}: '{match.emoji}' ({match.unicode_name})")
...
Examples:
  ...
  python emoji_detector.py --extensions .md .py  # Specific file types
    ...
    parser.add_argument('--target', '-t', type=Path, default=Path.cwd(),
                        help='Directory to scan (default: current directory)')
    parser.add_argument('--format', '-f', choices=['console', 'json', 'detailed'],
