...
class AIOSFolderComparisonVerifier:
    ...
    def perform_comprehensive_comparison(self) -> Dict[str, Any]:
        ...
        comparison_results["content_verification"] = (
            self._verify_critical_file_content()
        )
        ...
        comparison_results["safe_to_delete"] = self._determine_safe_deletion(
            comparison_results
        )
        comparison_results["deletion_recommendation"] = (
            self._generate_deletion_recommendation(comparison_results)
        )
    ...
    def _compare_folder_structures(self) -> Dict[str, Any]:
        ...
        structure_comparison["common_folders"] = list(
            backup_folders.intersection(archive_folders)
        )
        structure_comparison["backup_only_folders"] = list(
            backup_folders - archive_folders
        )
        structure_comparison["archive_only_folders"] = list(
            archive_folders - backup_folders
        )

        logger.info(f"[FOLDER] Structure comparison:")
        logger.info(f"   Common folders: {len(structure_comparison['common_folders'])}")
        logger.info(
            f"   Backup-only folders: {len(structure_comparison['backup_only_folders'])}"
        )
        logger.info(
            f"   Archive-only folders: {len(structure_comparison['archive_only_folders'])}"
        )
    ...
    def _get_folder_structure(self, root_folder: Path) -> Dict[str, List[str]]:
        ...
        for item in root_folder.rglob("*"):
            if item.is_dir():
                ...
                parent_path = (
                    str(item.parent.relative_to(root_folder))
    ...
    def _determine_safe_deletion(self, comparison_results: Dict[str, Any]) -> bool:
        ...
        critical_files_identical = (
            content_verification["identical_files"] >= 4
            and content_verification["different_files"] == 0
        )
        ...
        only_expected_unique = all(
            f in expected_unique_files for f in unique_to_archive
        )
        ...
        safe_to_delete = (
            critical_files_identical
            and only_expected_unique
            and no_important_backup_files
        )
    ...
    def _generate_deletion_recommendation(
        ...
        if comparison_results["safe_to_delete"]:
            return (
                "[CHECK] SAFE TO DELETE: Backup folder contains identical content to archive. "
            ...
            unexpected_archive_files = [
                f
                for f in comparison_results["unique_to_archive"]
            ...
            return (
                f"[WARNING] NOT SAFE TO DELETE: {', '.join(differences)}. "
    ...
    def display_comparison_results(self):
        ...
        print(
            f"  [FOLDER] Backup folder: {file_counts['backup_files']} files, {file_counts['backup_directories']} directories"
        )
        print(
            f"   Archive folder: {file_counts['archive_files']} files, {file_counts['archive_directories']} directories"
        )
        print(
            f"  [CHART] Difference: {file_counts['file_count_difference']} files, {file_counts['directory_count_difference']} directories"
        )
        print()
        ...
        if comparison_results["unique_to_archive"]:
            print(
                f"      Archive-only files: {', '.join(comparison_results['unique_to_archive'])}"
            )
        print()
