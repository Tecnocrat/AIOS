...
class AIOSDatabase:
    ...
    def add_backup(
        ...
        if not is_duplicate:
            ...
            logger.debug(
                f"New content stored: {content_hash[:8]}... ({file_size} bytes)"
            )
        else:
    ...
    def get_content_by_hash(self, content_hash: str) -> Optional[bytes]:
        ...
        cursor = self.conn.cursor()
        cursor.execute(
            "SELECT content FROM file_content WHERE content_hash = ?", (content_hash,)
        )
        row = cursor.fetchone()
    ...
    def get_deduplication_stats(self) -> Dict[str, Any]:
        ...
        space_saved_pct = (
            (space_saved / total_size_with_dups * 100)
            if total_size_with_dups > 0
            else 0
        )
    ...
    def add_spatial_metadata(self, folder_path: str, metadata: Dict[str, Any]) -> int:
        """
