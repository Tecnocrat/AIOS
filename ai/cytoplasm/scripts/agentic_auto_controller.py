...
from laboratory.scripts.agentic_instruction_generator import (
    AgenticInstructionGenerator, AgenticTask, TaskPriority
)
...
class AgenticAutoController:
    ...
    def analyze_and_trigger(self, quality_results: Dict[str, Any], 
                           force_agentic: bool = False) -> Dict[str, Any]:
        """Analyze results and determine if agentic auto mode should trigger"""
        
        # Generate agentic tasks from analysis
        agentic_tasks = self.instruction_generator.generate_from_quality_analysis(quality_results)
        
        # Decision logic for auto-triggering
        should_trigger = force_agentic or self._should_trigger_auto_mode(quality_results)
        ...
        result = {
            ...
            "ai_prompt_ready": self._generate_combined_ai_prompt(agentic_tasks) if should_trigger else None
        }
    ...
    def _should_trigger_auto_mode(self, quality_results: Dict[str, Any]) -> bool:
        ...
        integration_score = quality_results.get("integration_analysis", {}).get("integration_score", 1.0)
        if integration_score < 0.5:
            return True
        
        return False
    
    def _assess_risk(self, quality_results: Dict[str, Any], 
                    agentic_tasks: List[AgenticTask]) -> Dict[str, Any]:
        ...
        high_priority_tasks = [task for task in agentic_tasks 
        ...
        if len(high_priority_tasks) > 0:
            risk_score += 0.1
            risk_factors.append(f"High priority tasks: {len(high_priority_tasks)} tasks")
        ...
        return {
            ...
            "requires_confirmation": risk_level == "HIGH" and self.safety_settings["require_confirmation_for_high_risk"]
        }
    
    def _create_execution_plan(self, agentic_tasks: List[AgenticTask], 
                              risk_assessment: Dict[str, Any]) -> Dict[str, Any]:
        """Create detailed execution plan for agentic tasks"""
        
        # Sort tasks by priority
        sorted_tasks = sorted(agentic_tasks, 
        ...
        if self.safety_settings["incremental_processing"]:
            ...
            for task in sorted_tasks:
                ...
                if len(current_batch_files) + len(task.target_files) > self.safety_settings["max_files_per_batch"]:
                    if current_batch:
        ...
        return {
            ...
            "rollback_strategy": "individual_task" if risk_assessment["risk_level"] == "HIGH" else "batch_level"
        }
    
    def _perform_safety_checks(self, agentic_tasks: List[AgenticTask]) -> Dict[str, Any]:
        ...
        if not checks["test_suite_available"]:
            warnings.append("No automated test suite detected - manual validation required")
        ...
        return {
            ...
            "warnings": warnings,
            "recommendation": "PROCEED" if all(checks.values()) else "CAUTION_REQUIRED"
        }
...
Please execute this comprehensive refactoring session autonomously, following the incremental approach and safety protocols outlined above.
...
"""
    ...
    def _communicate_agentic_decision(self, decision_result: Dict[str, Any]):
        ...
        storage_message = InterCellularMessage(
            ...
            correlation_id=f"agentic_decision_{int(datetime.now().timestamp())}"
        )
        ...
        if decision_result["should_trigger_agentic"]:
            membrane_message = InterCellularMessage(
                ...
                timestamp=datetime.now(),
                correlation_id=f"ai_agent_prep_{int(datetime.now().timestamp())}"
            )
