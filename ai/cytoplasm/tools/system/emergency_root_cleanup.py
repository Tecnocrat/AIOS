...
class CoreEngineRootCleanup:
    ...
    def perform_emergency_cleanup(self) -> Dict[str, Any]:
        ...
        for item in root_items:
            if item.is_file():
                ...
                if item.name not in self.essential_root_dirs:
                    ...
                    cleanup_results["root_violations_found"] += 1
                    logger.warning(
                        f"[EMERGENCY] Unexpected directory in root: {item.name}"
                    )
        ...
        cleanup_results["cleanup_summary"] = {
            ...
            "fix_success_rate": cleanup_results["root_violations_fixed"]
            / max(cleanup_results["root_violations_found"], 1),
            "essential_files_preserved": len(cleanup_results["files_preserved"]),
            "tachyonic_categories_used": len(
                set(r["tachyonic_category"] for r in cleanup_results["files_relocated"])
            ),
        }
        ...
        logger.info(
            f"[EMERGENCY] Cleanup complete: {cleanup_results['root_violations_fixed']}/{cleanup_results['root_violations_found']} violations fixed"
        )
    ...
    def _relocate_file(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"[EMERGENCY] Relocated {file_path.name} to {category}/{new_name}"
            )
    ...
    def _analyze_file(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                file_path.suffix in [".json", ".md", ".txt"]
                and analysis["file_size"] < 10000
            ):
                try:
    ...
    def _store_cleanup_report(self, cleanup_results: Dict[str, Any]):
        ...
        report_path = (
            self.tachyonic_path
            / "emergency_cleanup"
            / f"EMERGENCY_CLEANUP_REPORT_{timestamp}.json"
        )
    ...
    def verify_root_cleanliness(self) -> Dict[str, Any]:
        ...
        verification["cleanliness_score"] = max(
            essential_score - cleanliness_penalty, 0.0
        )
        verification["root_is_clean"] = (
            verification["cleanliness_score"] >= 0.95 and unexpected_count == 0
        )
...
def main():
    ...
    print("=" * 70)
    print(
        f" Violations Found: {cleanup_results['cleanup_summary']['violations_found']}"
    )
    print(
        f" Violations Fixed: {cleanup_results['cleanup_summary']['violations_fixed']}"
    )
    print(
        f" Fix Success Rate: {cleanup_results['cleanup_summary']['fix_success_rate']:.1%}"
    )
