...
class AIOSHolographicMetadataSystem:
    ...
    def __init__(self, workspace_root: Path = None):
        ...
        self.file_type_history_file = "runtime/analysis/aios_file_type_history.json"
    ...
    def generate_folder_metadata(self, folder_path: Path) -> Dict[str, Any]:
        ...
        metadata = {
            ...
            "architectural_classification": self._classify_architectural_area(
                relative_path
            ),
    ...
    def _classify_architectural_area(self, relative_path: Path) -> Dict[str, Any]:
        ...
        for part in path_parts:
            ...
            for area_key, area_info in self.architectural_areas.items():
                if area_key in part_lower or any(
                    keyword.lower() in part_lower for keyword in area_info
                ):
    ...
    def _analyze_spatial_context(self, folder_path: Path) -> Dict[str, Any]:
        ...
        if folder_path.parent.exists():
            siblings = [
                p.name
                for p in folder_path.parent.iterdir()
                if p.is_dir() and p != folder_path
            ]

        # Get children directories
        children = (
            [p.name for p in folder_path.iterdir() if p.is_dir()]
        ...
        total_size = (
            sum(f.stat().st_size for f in folder_path.rglob("*") if f.is_file())
            if folder_path.exists()
            else 0
        )
        file_count = (
            len([f for f in folder_path.rglob("*") if f.is_file()])
            if folder_path.exists()
            else 0
        )
    ...
    def _analyze_folder_contents(self, folder_path: Path) -> Dict[str, Any]:
        ...
        return {
            ...
            "content_summary": {
                ...
                "dominant_type": (
                    max(files_by_type.keys(), key=lambda k: len(files_by_type[k]))
                    if files_by_type
                    else None
                ),
            },
    ...
    def _calculate_consciousness_metrics(self, folder_path: Path) -> Dict[str, Any]:
        ...
        return {
            ...
            "consciousness_level": (
                ...
                else "medium" if consciousness_score > 20 else "low"
            ),
        }
            ...
            for file_path in folder_path.iterdir():
                if file_path.is_file():
                    ...
                    if any(
                        ...
                        for pattern in ["main", "index", "entry", "__init__"]
                    ):
                        entry_points.append(file_path.name)
                    elif any(
                        pattern in file_name
                        for pattern in ["config", "settings", "setup"]
                    ):
    ...
    def _generate_ai_guidance(
        self, folder_path: Path, relative_path: Path
    ) -> Dict[str, Any]:
        ...
        for part in path_parts:
            ...
            for folder in self.workspace_root.rglob("*"):
                if (
                    folder.is_dir()
                    and related_pattern in folder.name.lower()
                    and folder != folder_path
                ):
                    try:
                        rel_path = folder.relative_to(self.workspace_root)
                        guidance["related_folders"].append(str(rel_path))
                        if (
                            len(guidance["related_folders"]) >= 5
                        ):  # Limit to 5 related folders
                            break
                    except ValueError:
                        continue
    ...
    def scan_workspace_file_types(self) -> Dict[str, Any]:
        ...
        sorted_extensions = dict(
            sorted(by_extension.items(), key=lambda x: x[1]["count"], reverse=True)
        )
        sorted_categories = dict(
            sorted(by_category.items(), key=lambda x: x[1]["count"], reverse=True)
        )
        sorted_architecture = dict(
            sorted(by_architecture.items(), key=lambda x: x[1]["count"], reverse=True)
        )
    ...
    def _analyze_programming_languages(self, extensions_data: Dict) -> Dict[str, Any]:
        ...
        sorted_languages = dict(
            sorted(languages.items(), key=lambda x: x[1]["count"], reverse=True)
        )
        ...
        return {
            ...
            "code_vs_other_ratio": round(
                total_code_files
                / max(sum(d["count"] for d in extensions_data.values()), 1),
                3,
            ),
        }
    ...
    def track_file_type_changes(self) -> Dict[str, Any]:
        ...
        cutoff_date = datetime.now() - timedelta(
            days=self.monitoring_config["history_retention_days"]
        )
        history_data = [
            entry
    ...
    def create_holographic_metadata(
        self, target_folder: Path = None, overwrite: bool = False
    ) -> Dict[str, str]:
    ...
    def scan_and_create_holographic_system(
        self, max_depth: int = 3, overwrite: bool = False
    ) -> Dict[str, Any]:
        ...
        for folder in self.workspace_root.rglob("*"):
            ...
            if folder_count % 50 == 0:
                logger.info(
                    f"📊 Scanned {folder_count} folders, processed {processed_count}..."
                )
        ...
        index_data = {
            ...
            "system_stats": {
                ...
                "coverage_percentage": (
                    (len(all_files) / max(folder_count, 1)) * 100
                    if folder_count > 0
                    else 0
                ),
            },
        ...
        logger.info(
            f"   Coverage: {index_data['system_stats']['coverage_percentage']:.1f}%"
        )
    ...
    def _should_create_metadata(self, folder: Path) -> bool:
        ...
        if has_significant_content:
            logger.debug(
                f"📁 Significant content folder: {folder} ({file_count} files, {subfolder_count} subfolders)"
            )
...
def main():
    ...
    parser.add_argument(
        "--create-system", action="store_true", help="Create full holographic system"
    )
    parser.add_argument(
        "--create-folder", type=Path, help="Create metadata for specific folder"
    )
    parser.add_argument(
        "--read-metadata", type=Path, help="Read metadata for specific folder"
    )
    parser.add_argument(
        "--scan-file-types", action="store_true", help="Scan workspace file types"
    )
    parser.add_argument(
        "--track-changes", action="store_true", help="Track file type changes over time"
    )
    parser.add_argument(
        "--file-type-report",
        action="store_true",
        help="Generate comprehensive file type report",
    )
    parser.add_argument(
        "--max-depth", type=int, default=3, help="Maximum folder depth to process"
    )
    parser.add_argument(
    ...
    if args.create_system:
        result = system.scan_and_create_holographic_system(
            args.max_depth, args.overwrite
        )
        ...
        if result:
            print(
                f"✅ Metadata {'updated' if args.overwrite else 'created'} for: {args.create_folder}"
            )
        else:
        ...
        report_file = (
            system.workspace_root / "runtime/analysis/aios_file_type_report.json"
        )
        report_data = {
            ...
            "report_summary": {
                ...
                "languages_detected": len(
                    scan_result["language_analysis"]["languages"]
                ),
                "monitoring_active": track_result["monitoring_active"],
                "significant_changes": len(
                    track_result["significant_changes"].get("significant_changes", [])
                ),
            },
        }
