...
def test_ai_cells_integration():
    ...
    assert manager.execute_workflow(workflow.workflow_id, training_data), "Workflow execution failed"
    # Wait for completion
    import time
    while True:
        ...
        if status and status.status in ["completed", "failed"]:
            break
        time.sleep(1)
    assert status.status == "completed", f"Workflow did not complete: {status.status}"
