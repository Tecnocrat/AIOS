...
if (-not (Test-Path $aiStatePath)) {
    Write-Host "⚠️  AI Visual Feedback not found - creating requirement" -ForegroundColor Yellow
...
if ($objectiveFiles.Count -gt 0) {
    ...
    foreach ($objFile in $objectiveFiles) {
        try {
            ...
            Write-Host "   ⚠️  Error reading objective: $($objFile.Name)" -ForegroundColor Yellow
        }
    }
}
...
if (Test-Path $sessionFile) {
    try {
        ...
        Write-Host "⚠️  Could not parse session metadata" -ForegroundColor Yellow
    }
}
...
if (-not (Test-Path "CHANGELOG.md") -and -not (Test-Path "docs/CHANGELOG.md") -and -not $Force) {
    Write-Host "⚠️  Creating required CHANGELOG.md for governance compliance" -ForegroundColor Yellow
