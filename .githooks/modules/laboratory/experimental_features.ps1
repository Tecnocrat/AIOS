    ...
    if (Test-Path $file) {
        ...
        if ($content) {
            ...
            if ($trailingWhitespaceCount -gt 0) {
                Write-Host "        $trailingWhitespaceCount lines with trailing whitespace" -ForegroundColor Yellow
                $totalErrors += $trailingWhitespaceCount
            }
            if ($longLineCount -gt 0) {
                Write-Host "        $longLineCount lines too long (>79 chars)" -ForegroundColor Yellow
