...
function Write-JsonLog {
  ...
  } catch { Write-Host "⚠️ Hook logging failed: $($_.Exception.Message)" -ForegroundColor Yellow }
}
...
function Get-Staged {
  ...
  $map = @{}
  foreach($line in $statusRaw -split "`n"){ if(-not $line.Trim()){continue}; $parts=$line -split "\s+"; $map[$parts[-1]] = $parts[0] }
...
# Bypass check
if ($env:AIOS_HOOK_BYPASS -eq '1') { Write-Host '⚠️ Hook bypass active via AIOS_HOOK_BYPASS=1' -ForegroundColor Yellow; Write-JsonLog -Status 'bypass' -Data @{ reason='env var'; user=$env:USERNAME }; exit 0 }
...
if ($violations.Count -eq 0) {
  if ($results.Contains('criticality_warn') -and $results.criticality_warn) {
    Write-Host "⚠️ Core file modifications (review advised): $($results.criticality_warn -join ', ')" -ForegroundColor Yellow
  }
