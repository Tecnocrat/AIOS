...
async def test_ai_core():
    ...
    try:
        ...
        for command in test_commands:
            ...
            print(
                f"   Result: {result.get('status', 'unknown')} - {result.get('intent', 'unknown')}"
            )
        ...
        print(
            f"   CPU prediction: {len(prediction.get('predictions', []))} data points"
        )

        # Test automation
        print("\n5. Testing automation...")
        automation_result = await ai_core.execute_automation(
            {"id": "test_task", "type": "generic", "params": {"test": True}}
        )
...
async def test_individual_managers():
    ...
    try:
        ...
        learn_result = await learn_manager.update(
            {"type": "general", "data": {"test": "learning data"}}
        )
