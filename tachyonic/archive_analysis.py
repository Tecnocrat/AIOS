...
def analyze_archive_intelligence():
    ...
    if archival_log.exists():
        try:
            ...
            print(f"⚠️  Could not load archival log: {e}")
    
    print("📊 ARCHIVE INTELLIGENCE SUMMARY:")
    print("-" * 50)
    print(f"🗂️  Total archive folders: {intelligence_summary['total_folders']}")
    ...
    print(f"⚛️  Quantum/Bosonic data: {intelligence_summary['quantum_data']} files")
    ...
    if archival_data and isinstance(archival_data, list) and len(archival_data) > 0:
        print(f"\n🏗️  EVOLUTIONARY ASSEMBLER ANALYSIS:")
    ...
    if bosonic_dir.exists():
        ...
        print(f"   ⚛️  Bosonic Substrate: {len(bosonic_files)} quantum entanglement records")
