...
# ------------------
def clear_codebase_folder(folder_path):
    """
...
def extract_zip_to_folder(zip_path, output_folder):
    ...
    Args:
        ...
        output_folder (str): The folder where the ZIP contents will be extracted.
    """
...
if __name__ == "__main__":
    ...
    print("\nStep 12: Backing up the entire C:\\Dev codebase...")
    codebase_folder = os.path.join(ADB_DIR, "knowledge_base", "codebase")
    clear_codebase_folder(codebase_folder)  # Clear the existing codebase folder

    zip_path = os.path.join(ADB_DIR, "knowledge_base", "codebase_backup.zip")
    extract_python_files_to_zip("C:\\Dev", zip_path)  # Create a ZIP of the C:\Dev folder

    extract_zip_to_folder(zip_path, codebase_folder)  # Extract the ZIP into the codebase folder

    print("\nStep 13: Explaining a Python code snippet...")
    code_snippet = """
    def greet(name):
      return f'Hello, {name}!'
    """
